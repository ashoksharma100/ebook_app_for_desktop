...
var readerApp = {
		...
		for(var key in appdb.ebook) {
			...
			if(key != "zother") {
				...
				for(var i=0; i<mItem.data.length; i++) {
					...
					} else if(mItem["link"] == "audio") {
						var isPage = (typeof item['page'] == 'undefined' ? null : item['page']);
						var onClick = "onClick=\"openAudio(\'" + item['path'] +  "\')\"";
						if(isPage != null) {
							onClick = "onClick=openAudio(\'" + item['path'] + "\'," + item['page'] + ") ";
						}
						linkHref = onClick + 'data-video-mp4="' + item['path'] + '" data-src="#popupVideo"';
						classList = "audio-fancybox";
				...
				for(var i=0; i<bookmarksList.length; i++) {
					bookingHTML += '<li>\
                            <a href="#" onClick="readerApp.turnPage(' + bookmarksList[i] + ')">\
                                <div>\
                                    <i class="fa fa-chevron-right fa-fw"></i> Page ' + bookmarksList[i] + '\
                                </div>\
                            </a>\
                        </li>';
			...
			for(var i=0; i<dataObj.length; i++) {
				$highlightContainer.append('<li>\
					<a href="#" onClick="readerApp.turnPage(' + dataObj[i] + ')">\
						<div>\
							<i class="fa fa-chevron-right fa-fw"></i> <b>Page ' + dataObj[i] + '</b> \
						</div>\
					</a>\
				</li>');
			}
	...
	'prepareNoteAdd': function(_self) {
		...
		for(var i=0; i<pages.length; i++) {
			selectHTML += '<option value="' + pages[i] + '">Page ' + pages[i] + '</option>'
		}
...
function openAudio(url, pageNum) {
	
	playAudio(url);
	if(typeof pageNum != "undefined") {
		readerApp.turnPage(pageNum);
	}
}
